...
class B_blog extends MX_Controller {
	...
	function post_edit(){
		...
		if(!$post = modules::run('blog/api_post_getbyid', $id)):
			return $this->dodol_theme->not_found();
		endif;
		
		if($pub = $this->input->post('publish') || $sav = $this->input->post('save')):
			$data = ddl_post_filter('bl_');
				if($new = modules::run('blog/api_post_update',$id , $data) ) :
					$this->messages->add('Success Update Post with title '.$new->title, 'success');
					redirect('backend/blog/b_blog/post_browse');
				else:
					$this->messages->add('somthing wrong, please try again', 'warning');
					return false;
	...
	function post_delete(){
		$id = $this->uri->segment(5);
		if($item =modules::run('blog/api_post_delete', $id)) :
			$this->messages->add('Success delete Post with title '.$item->title, 'success');
			redirect('backend/blog/b_blog/post_browse');
		else:
			$this->messages->add('somthing wrong, please try again', 'warning');
			redirect('backend/blog/b_blog/post_browse');
