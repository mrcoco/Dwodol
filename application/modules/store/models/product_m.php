...
class Product_m extends CI_Model {
	...
	function deleteMedia($id){
		$q = $this->getMediaById($id);
			if($q){
				$path = './assets/modules/store/product_img/'.$q->path;
				if(file_exists($path)){
					$del = unlink($path);
					if ($del){
						return true;
	...
	function renameMedia($id, $new){
		...
		$q = $this->getMediaById($id);
			if($q){
				$path = './assets/modules/store/product_img/'.$q->path;
				$ext  = pathinfo($path);
				$new  = './assets/modules/store/product_img/'.$new_name.'.'.$ext['extension'];
				if(file_exists($path)){
					$ren = rename($path, $new);
					if ($ren){
	...
	function browse($param){
		...
		if($select = element('select', $param))
		$this->db->select($select);
		...
		if($id_not = element('id_not', $param))
		$this->db->where_not_in('prod.id', $id_not);

		if($id_is = element('id_is', $param))
		$this->db->where_in('prod.id', $id_is);
	...
	function uploadfile($field, $file_name){
		$this->load->library('upload');
		$config = array(
			'upload_path' 	=> './assets/modules/store/product_img/',
	...
	function media_create($data){
		if($q = $this->db->insert('store_product_media',$data)){
			return $this->media_getbyid($this->db->insert_id());
		}else{
			return false;
		}
	...
	function media_update($id, $data){
		if($this->media_getbyid($id)){
			$this->db->where('id', $id);
			if($this->db->update('store_product_media', $data)){
				return $this->media_getbyid($id);
			}else{
				return false;
			}
		}else{
	...
	function relation_create($data){
		...
		if($pre->num_rows() == 0){
			...
			if($q){
				return $this->relation_getbyid($this->db->insert_id());
			}else{
				return false;
			}
		}else{
