...
class Template
{
	 ...
	 */
	function Template($config = array())
	{
	...
	public function build($view, $data = array(), $return = FALSE)
	{
		...
		if( element('pT', $data) && !element('pH', $data) ):
		$data['pH'] = element('pT', $data);
		elseif ( !element('pT', $data) && element('pH', $data) ):
		$data['pT'] = element('pH', $data);
		elseif( !element('pT', $data) && !element('pH', $data) ):
		$data['pT'] = ''; $data['pH'] = '';
		endif;
	...
	{
		$debug = '<div class="debug_theme_loc box2">';
		...
		$this->_ci->bug->send(array('theme_loc' => $debug));
