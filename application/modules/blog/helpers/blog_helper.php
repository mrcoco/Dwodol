...
function post_next_link($id){
	$ci =& get_instance();
	$ci->db->select('a.slug as slug, a.title, a.id as ID');
	$ci->db->where('a.id >', $id);
	$ci->db->where('a.status', 'publish');
	$ci->db->join('blog_category b', 'b.id = a.cat_id');
	$ci->db->join('user c', 'c.id = a.author');
	$ci->db->order_by('a.id', 'ASC');
	$q = $ci->db->get('blog_post a',  1);
	if($q->num_rows() == 1){
		echo '<span class="nav"><a class="next" href="'.site_url('blog/read/'.$q->row()->slug).'" title="'.$q->row()->title.'" >Next Post</a></span>';
	}
}
function post_prev_link($id){
	$ci =& get_instance();
	$ci->db->select('a.slug as slug, a.title, a.id as ID');
	$ci->db->where('a.id <', $id);
	$ci->db->where('a.status', 'publish');
	...
	$q = $ci->db->get('blog_post a',  1);
	if($q->num_rows() == 1){
		echo '<span class="nav"><a class="prev" href="'.site_url('blog/read/'.$q->row()->slug).'" title="'.$q->row()->title.'" >Previous Post</a></span>';
	}
}
function post_comment_count($id){
	if($q = modules::run('blog/api_comment_browse', array('post_id' => $id, 'result' => false))):
		return element('num_rows', $q);
	else:
		$num_rows = 0;
	return $num_rows;
	endif;
