...
class Blog_m extends CI_Model {
	...
	function post_update($id, $data){
		...
		$data['slug'] = str_replace(' ', '_', element('title', $data));
		$prev_data = $this->post_getbyid($id);
		if($prev_data->title != element('title', $data)) :
		$this->db->where('title', element('title', $data));
		$pre = $this->db->get('blog_post');
			if($pre->num_rows() > 0) $data['slug'] = str_replace(' ', '_', element('title', $data)).'_'.$pre->num_rows()+1;
		endif;
		
		$this->db->where('id', $id);
		if($this->db->update('blog_post', $data)):
			return $this->post_getbyid($id);
		else:
			return false;
		endif;
	...
	function post_getbyslug($slug){	
		$this->db->select('*');
		$this->db->select('b.name as cat_name, b.id as id_cat, a.id as ID, a.slug as slug');
	...
	function post_browse($param){
		...
		if($src = element('src', $param)):
			$this->db->like('a.title', $src);
		endif;
		$this->db->select('*');
		$this->db->select('a.id as ID, a.slug as slug, b.name as cat_name');
		$this->db->join('blog_category b', 'a.cat_id=b.id');
		$this->db->join('user c', 'c.id = a.author');
		$this->db->order_by('a.id', 'DESC');
		$this->dodol->db_calc_found_rows();
		$q = $this->db->get('blog_post a', element('limit', $param), element('start', $param));
		if($q->num_rows() > 0):
	...
	function comment_browse($data){
		if(element('result' , $data) == true) :
		$this->db->select('*');
		else:
		$this->db->select('id');
		endif;
		if(element('post_id', $data)) : 
		$this->db->where('post_id', element('post_id', $data)); 
		endif;
		$this->dodol->db_calc_found_rows();
		$q = $this->db->get('blog_comment',  element('limit', $data), element('start', $data));
		if($q->num_rows() >0):
			return array('comments' => $q->result(), 'num_rows' => $this->dodol->db_found_rows());
		else:
			return false;
		endif;
	}
