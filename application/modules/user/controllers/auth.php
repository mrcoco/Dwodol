...
class Auth extends MX_Controller {
	...
	function backend_login(){
		...
		if($this->session->userdata('back_login_data') && modules::run('user/auth/userRoleCheck', 'owner')){
			redirect('backend');
		}else{
		...
		$this->dodol_theme->render()->set_layout('login_back')->build('page/backend/backend_login_v');
		
		if($this->input->post('login')){
			$q = $this->load->library('dodol/dodol_auth')->do_login($this->input->post('email'), $this->input->post('password'), 'backend');
			if($q){
