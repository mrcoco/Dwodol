...
{					
	$.fn.sideswap = function(options)
	{
		$.fn.sideswap.defaults = 
		{
			...
			animate_in_type : 'fade',
			animate_in_opt : {},
			animate_out_type : 'fade',
			animate_out_opt : {}
		};
		...
		return this.each(function()
		{
			...
			function rotate(direction)
			{
				...
				if(count > 1)
				{
					$current_image.hide(opts.animate_in_type, opts.animate_in_opt, opts.duration);
					
					switch(direction)
					{
						case -1:
							$previous_image.show(opts.animate_in_type, opts.animate_in_opt, opts.duration,function()
							{
								clicked = false;
							});
							break;
						default:
							$next_image.show(opts.animate_in_type, opts.animate_in_opt, opts.duration,function()
							{
								clicked = false;
							});
							break;
			...
			{
				var $toolNav =$('<DIV></DIV>');
				$toolNav.addClass('slideNavTool');
				var $div = $('<DIV></DIV>'), /* CREATE THE DIV OBJECT */
						$next = $div.clone().addClass('sideswap_nav sideswap_next').html(opts.next).appendTo($toolNav).hide(), /* CREATE THE NEXT OBJECT */
						$previous = $div.clone().addClass('sideswap_nav sideswap_previous').html(opts.previous).appendTo($toolNav).hide(); /* CREATE THE PREVIOUS OBJECT */
				$toolNav.appendTo($parent);
				$next.click(function()
				{
					if(!clicked)
					{
