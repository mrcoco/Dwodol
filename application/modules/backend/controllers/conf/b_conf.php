...
class B_conf extends MX_Controller {
	...
	function create(){
		...
		$this->dodol_theme->render()->build('page/conf/create_v', $render);
	...
	function update(){
		...
		if(!$conf) return $this->dodol_theme->not_found();
		$render['pT'] = 'Update Configuration -'.$conf->name;
		$render['pH'] = 'Update Configuration -'.$conf->name;
		$render['conf'] = $conf;
		$this->dodol_theme->render()->build('page/conf/update_v', $render);
	...
	function Browse(){
		...
		$this->dodol_theme->render()->build('page/conf/browse_v', $render);
	}
	function delete(){
		$id = $this->uri->segment(5);
